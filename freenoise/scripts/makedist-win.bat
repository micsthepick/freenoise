...
if %1 == 1 (echo Making freenoise Windows DEMO VERSION distribution ...) else (echo Making freenoise Windows FULL VERSION distribution ...)
...
REM - msbuild freenoise-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild freenoise.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild freenoise.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\freenoise.aaxplugin\Contents\Win32\freenoise.aaxplugin --out .\build-win\aax\bin\freenoise.aaxplugin\Contents\Win32\freenoise.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\freenoise.aaxplugin\Contents\x64\freenoise.aaxplugin --out .\build-win\aax\bin\freenoise.aaxplugin\Contents\x64\freenoise.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\freenoise.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\freenoise.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "freenoise Installer" ".\installer\freenoise Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\freenoise Installer.exe" ".\installer\freenoise Demo Installer.exe"
REM -del ".\installer\freenoise Installer.exe"
REM -)
