...
def  main():
  ...
  installer = "\installer\freenoise Installer.exe"
   
  if demo:
    installer = "\installer\freenoise Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\freenoise manual.pdf" 
  ]
  ...
  ZIPNAME = "freenoise-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "freenoise-v" + FULLVERSIONSTR + "-win-demo.zip"
